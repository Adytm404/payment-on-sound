...
export default function AdminUserDetailPage() {
  ...
  return (
    <div className="space-y-6 p-5 lg:p-8">
      ...
      <section className="grid gap-5 xl:grid-cols-[1fr_0.8fr]">
        ...
        <div className="rounded-[1.75rem] border border-white/10 bg-white/[0.04] p-6">
          ...
          <div className="mt-4 flex gap-3">
            <button disabled={saving} onClick={() => setConfirmAction("deactivate")} className="rounded-2xl bg-red-600 px-4 py-3 text-sm font-black disabled:opacity-50">Nonaktifkan</button>
            <button disabled={saving} onClick={() => saveStatus(true)} className="rounded-2xl bg-emerald-600 px-4 py-3 text-sm font-black disabled:opacity-50">Aktifkan / Simpan Note</button>
          </div>
        </div>
      </section>
      ...
      <ConfirmDialog open={confirmAction === "deactivate"} title="Nonaktifkan User?" description={`User ${data.user.name} tidak akan bisa membuat transaksi atau mengubah settings.`} confirmLabel="Nonaktifkan" tone="danger" loading={saving} onConfirm={() => saveStatus(false)} onCancel={() => setConfirmAction(null)} />
    </div>
  );
}
