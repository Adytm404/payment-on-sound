...
export default function AdminTransactionsPage() {
  ...
  function load() {
    adminApi.transactions({ search, status, page, limit: 50 }).then((res) => { setRows(res.data); setSummary(res.summary); setPagination(res.pagination); });
  }
  ...
  return (
    <div className="space-y-5 p-5 lg:p-8">
      <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
        <div>
          <p className="text-xs uppercase tracking-[0.28em] text-red-300">Billing</p>
          <h2 className="text-3xl font-black">Upgrade & Langganan Pro</h2>
        </div>
        <div className="flex gap-3">
          <input className="input max-w-sm bg-white text-ink" placeholder="Cari order/merchant/nominal" value={search} onChange={(e) => setSearch(e.target.value)} />
          <select className="input w-44 bg-white text-ink" value={status} onChange={(e) => setStatus(e.target.value)}>
            <option value="all">Semua</option><option value="pending">Pending</option><option value="paid">Terbayar</option><option value="failed">Gagal</option><option value="expired">Expired</option><option value="cancelled">Batal</option>
          </select>
        </div>
      </div>
